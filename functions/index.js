...
exports.teacherDashboard = onCall(callableOptions, async function (request) {
  const auth = request.auth;
  const data = request.data || {};
  const action = data.action || "";

  verifyTeacherDashboardCaller(auth);

  if (action === "listStudents") {
    return listTeacherDashboardStudents(data, auth);
  }

  throw new HttpsError("invalid-argument", "Unknown teacher dashboard action.");
});
...
async function listTeacherDashboardStudents(data, auth) {
  const db = admin.firestore();
  const caller = await loadTeacherDashboardCaller(db, auth);
  const scope = resolveTeacherDashboardScope(caller.profile, caller.roles);
  const requestedClassIds = readRequestedScopeIds(data.classId || data.classIds);
  const requestedLocationIds = readRequestedScopeIds(data.locationId || data.locationIds || data.schoolId || data.schoolIds);
  const effectiveClassIds = filterAuthorizedScope(requestedClassIds, scope.classIds, caller.isGlobalAdmin, "class");
  const effectiveLocationIds = filterAuthorizedScope(requestedLocationIds, scope.locationIds, caller.isGlobalAdmin, "location");
  const students = [];
  const queryErrors = [];
  const debug = {
    requestedClassIds: requestedClassIds,
    requestedLocationIds: requestedLocationIds,
    resolvedClassIds: effectiveClassIds,
    resolvedLocationIds: effectiveLocationIds,
    queryPaths: []
  };

  if (!caller.isGlobalAdmin && effectiveClassIds.length === 0 && effectiveLocationIds.length === 0) {
    throw new HttpsError("failed-precondition", "No authorized classes or locations were found for this teacher dashboard account.");
  }

  await appendTeacherDashboardStudentsByClass(db, students, queryErrors, debug, effectiveClassIds);

  if (students.length === 0 || effectiveClassIds.length === 0) {
    await appendTeacherDashboardStudentsByLocation(db, students, queryErrors, debug, effectiveLocationIds);
  }

  return buildTeacherDashboardStudentsResponse(students, queryErrors, debug, data);
}

function verifyTeacherDashboardCaller(auth) {
  if (!auth || !auth.uid) {
    throw new HttpsError("unauthenticated", "Sign in to use the teacher dashboard.");
  }
}

async function loadTeacherDashboardCaller(db, auth) {
  const authUid = auth.uid;
  const tokenRoles = readCallerRoles(auth);
  const profileLookup = await readCallerProfile(db, authUid);
  const profile = profileLookup.profile;
  const roles = mergeRoleLists(tokenRoles, readRoles(profile));
  const allowedRoles = ["teacher", "assistant", "schoolAdmin", "platformAdmin", "superAdmin"];
  const isAllowed = roles.some(function (role) { return allowedRoles.indexOf(role) !== -1; });

  if (!isAllowed) {
    throw new HttpsError("permission-denied", "Only teachers, assistants, or admins can use the teacher dashboard.");
  }

  if (!profileLookup.found && roles.indexOf("platformAdmin") === -1 && roles.indexOf("superAdmin") === -1) {
    throw new HttpsError("failed-precondition", "Teacher dashboard profile was not found.");
  }
  ...
  return {
    ...
    profile: profile,
    roles: roles,
    isGlobalAdmin: roles.indexOf("platformAdmin") !== -1 || roles.indexOf("superAdmin") !== -1
  };
}

async function readCallerProfile(db, authUid) {
  const directDoc = await db.collection("users").doc(authUid).get();

  if (directDoc.exists) {
    return {
      found: true,
      id: directDoc.id,
      profile: Object.assign({ id: directDoc.id }, directDoc.data() || {})
    };
  }

  const authUidSnapshot = await db.collection("users").where("authUid", "==", authUid).limit(1).get();
  let profile = null;
  let profileId = "";

  authUidSnapshot.forEach(function (profileDoc) {
    if (!profile) {
      profileId = profileDoc.id;
      profile = Object.assign({ id: profileDoc.id }, profileDoc.data() || {});
    }
  });
...
function resolveTeacherDashboardScope(profile, roles) {
  const isGlobalAdmin = roles.indexOf("platformAdmin") !== -1 || roles.indexOf("superAdmin") !== -1;
  const locationIds = readTeacherLocationIds(profile || {}, readFirstTeacherLocationId(profile || {}));

  return {
    classIds: isGlobalAdmin ? [] : readTeacherClassIds(profile || {}),
    locationIds: isGlobalAdmin ? [] : locationIds
  };
}

function readRequestedScopeIds(value) {
  return readIdList(value).slice(0, 30);
}

function filterAuthorizedScope(requestedIds, authorizedIds, isGlobalAdmin, scopeName) {
  if (isGlobalAdmin) {
    return requestedIds;
  }

  if (!Array.isArray(requestedIds) || requestedIds.length === 0) {
    return authorizedIds.slice();
  }

  const filtered = requestedIds.filter(function (id) {
    return authorizedIds.indexOf(id) !== -1;
  });

  if (filtered.length !== requestedIds.length) {
    throw new HttpsError("permission-denied", "Requested " + scopeName + " is outside this dashboard account's scope.");
  }

  return filtered;
}

async function appendTeacherDashboardStudentsByClass(db, students, queryErrors, debug, classIds) {
  const chunks = chunkValues(classIds, 10);
  let index = 0;

  while (index < chunks.length) {
    await appendTeacherDashboardStudentQuery(students, queryErrors, debug, db.collection("users").where("classId", "in", chunks[index]), "users where classId in teacherClassIds");
    await appendTeacherDashboardStudentQuery(students, queryErrors, debug, db.collection("users").where("classIds", "array-contains-any", chunks[index]), "users where classIds array-contains-any teacherClassIds");
    await appendTeacherDashboardStudentQuery(students, queryErrors, debug, db.collection("users").where("assignedClassIds", "array-contains-any", chunks[index]), "users where assignedClassIds array-contains-any teacherClassIds");
    index = index + 1;
  }
}

async function appendTeacherDashboardStudentsByLocation(db, students, queryErrors, debug, locationIds) {
  const chunks = chunkValues(locationIds, 10);
  let index = 0;

  while (index < chunks.length) {
    await appendTeacherDashboardStudentQuery(students, queryErrors, debug, db.collection("users").where("locationId", "in", chunks[index]), "users where locationId in teacherLocationIds");
    await appendTeacherDashboardStudentQuery(students, queryErrors, debug, db.collection("users").where("primaryLocationId", "in", chunks[index]), "users where primaryLocationId in teacherLocationIds");
    await appendTeacherDashboardStudentQuery(students, queryErrors, debug, db.collection("users").where("schoolId", "in", chunks[index]), "users where schoolId in teacherLocationIds");
    await appendTeacherDashboardStudentQuery(students, queryErrors, debug, db.collection("users").where("locationIds", "array-contains-any", chunks[index]), "users where locationIds array-contains-any teacherLocationIds");
    await appendTeacherDashboardStudentQuery(students, queryErrors, debug, db.collection("users").where("schoolIds", "array-contains-any", chunks[index]), "users where schoolIds array-contains-any teacherLocationIds");
    index = index + 1;
  }
}

async function appendTeacherDashboardStudentQuery(students, queryErrors, debug, studentsQuery, queryPath) {
  debug.queryPaths.push(queryPath);

  try {
    const snapshot = await studentsQuery.limit(250).get();
    snapshot.forEach(function (studentDoc) {
      const student = studentDoc.data() || {};

      if (hasStudentRole(student) && isActiveStudent(student)) {
        addUniqueSanitizedStudent(students, sanitizeStudent(studentDoc.id, student));
      }
    });
  } catch (error) {
    queryErrors.push(queryPath + " failed: " + readErrorMessage(error));
  }
}

function addUniqueSanitizedStudent(students, student) {
  if (!student || !student.id) {
    return;
  }

  if (!students.some(function (existingStudent) { return existingStudent.id === student.id; })) {
    students.push(student);
  }
}

function buildTeacherDashboardStudentsResponse(students, queryErrors, debug, data) {
  ...
  if (queryErrors.length > 0) {
    response.warnings = queryErrors.map(function (queryError) {
      return {
        code: "TEACHER_STUDENT_QUERY_FAILED",
        message: queryError
      };
    });
  }
  ...
  if (isDebugRequest(data)) {
    response.debug = Object.assign({}, debug, {
...
function mergeRoleLists() {
  const roles = [];
  Array.prototype.slice.call(arguments).forEach(function (roleList) {
    (Array.isArray(roleList) ? roleList : []).forEach(function (role) {
