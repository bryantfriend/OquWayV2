import { getExternalTaskDefaultContent } from "../../externalTask.schema.js?v=1.1.226-learning-activity-files";

export function renderTemplate(activityContext) {
  var container = activityContext && activityContext.container ? activityContext.container : null;
  var content = activityContext && activityContext.content ? activityContext.content : getExternalTaskDefaultContent();

  if (!container) {
    return;
  }

  container.innerHTML = '<style>' + buildExternalTaskStandardCss() + '</style>'
    + '<article class="oqu-template-preview-shell">'
    + '<div class="oqu-template-preview-kicker">External Task</div>'
    + '<h2>' + escapeHtml(content.title || content.heading || content.word || content.phrase || content.missionTitle || "External Task") + '</h2>'
...
function buildExternalTaskStandardCss() {
  return ""
