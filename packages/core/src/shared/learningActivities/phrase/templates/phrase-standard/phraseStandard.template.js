import { getPhraseDefaultContent } from "../../phrase.schema.js?v=1.1.226-learning-activity-files";

export function renderTemplate(activityContext) {
  var container = activityContext && activityContext.container ? activityContext.container : null;
  var content = activityContext && activityContext.content ? activityContext.content : getPhraseDefaultContent();

  if (!container) {
    return;
  }

  container.innerHTML = '<style>' + buildPhraseStandardCss() + '</style>'
    + '<article class="oqu-template-preview-shell">'
    + '<div class="oqu-template-preview-kicker">Phrase</div>'
    + '<h2>' + escapeHtml(content.title || content.heading || content.word || content.phrase || content.missionTitle || "Phrase") + '</h2>'
...
function buildPhraseStandardCss() {
  return ""
