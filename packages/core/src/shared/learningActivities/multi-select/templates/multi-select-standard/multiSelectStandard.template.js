import { getMultiSelectDefaultContent } from "../../multiSelect.schema.js?v=1.1.226-learning-activity-files";

export function renderTemplate(activityContext) {
  var container = activityContext && activityContext.container ? activityContext.container : null;
  var content = activityContext && activityContext.content ? activityContext.content : getMultiSelectDefaultContent();

  if (!container) {
    return;
  }

  container.innerHTML = '<style>' + buildMultiSelectStandardCss() + '</style>'
    + '<article class="oqu-template-preview-shell">'
    + '<div class="oqu-template-preview-kicker">Multi Select</div>'
    + '<h2>' + escapeHtml(content.title || content.heading || content.word || content.phrase || content.missionTitle || "Multi Select") + '</h2>'
...
function buildMultiSelectStandardCss() {
  return ""
