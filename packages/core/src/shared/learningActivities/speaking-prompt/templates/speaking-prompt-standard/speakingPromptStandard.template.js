import { getSpeakingPromptDefaultContent } from "../../speakingPrompt.schema.js?v=1.1.226-learning-activity-files";

export function renderTemplate(activityContext) {
  var container = activityContext && activityContext.container ? activityContext.container : null;
  var content = activityContext && activityContext.content ? activityContext.content : getSpeakingPromptDefaultContent();

  if (!container) {
    return;
  }

  container.innerHTML = '<style>' + buildSpeakingPromptStandardCss() + '</style>'
    + '<article class="oqu-template-preview-shell">'
    + '<div class="oqu-template-preview-kicker">Speaking Prompt</div>'
    + '<h2>' + escapeHtml(content.title || content.heading || content.word || content.phrase || content.missionTitle || "Speaking Prompt") + '</h2>'
...
function buildSpeakingPromptStandardCss() {
  return ""
