import { getIntroCardDefaultContent } from "../../introCard.schema.js?v=1.1.226-learning-activity-files";

export function renderTemplate(activityContext) {
  var container = activityContext && activityContext.container ? activityContext.container : null;
  var content = activityContext && activityContext.content ? activityContext.content : getIntroCardDefaultContent();

  if (!container) {
    return;
  }

  container.innerHTML = '<style>' + buildIntroCardStandardCss() + '</style>'
    + '<article class="oqu-template-preview-shell">'
    + '<div class="oqu-template-preview-kicker">Intro Card</div>'
    + '<h2>' + escapeHtml(content.title || content.heading || content.word || content.phrase || content.missionTitle || "Intro Card") + '</h2>'
...
function buildIntroCardStandardCss() {
  return ""
