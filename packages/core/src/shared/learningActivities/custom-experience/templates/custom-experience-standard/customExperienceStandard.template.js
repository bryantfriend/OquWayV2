import { getCustomExperienceDefaultContent } from "../../customExperience.schema.js?v=1.1.226-learning-activity-files";

export function renderTemplate(activityContext) {
  var container = activityContext && activityContext.container ? activityContext.container : null;
  var content = activityContext && activityContext.content ? activityContext.content : getCustomExperienceDefaultContent();

  if (!container) {
    return;
  }

  container.innerHTML = '<style>' + buildCustomExperienceStandardCss() + '</style>'
    + '<article class="oqu-template-preview-shell">'
    + '<div class="oqu-template-preview-kicker">Custom Experience</div>'
    + '<h2>' + escapeHtml(content.title || content.heading || content.word || content.phrase || content.missionTitle || "Custom Experience") + '</h2>'
...
function buildCustomExperienceStandardCss() {
  return ""
