import { getListeningDefaultContent } from "../../listening.schema.js?v=1.1.226-learning-activity-files";

export function renderTemplate(activityContext) {
  var container = activityContext && activityContext.container ? activityContext.container : null;
  var content = activityContext && activityContext.content ? activityContext.content : getListeningDefaultContent();

  if (!container) {
    return;
  }

  container.innerHTML = '<style>' + buildListeningStandardCss() + '</style>'
    + '<article class="oqu-template-preview-shell">'
    + '<div class="oqu-template-preview-kicker">Listening</div>'
    + '<h2>' + escapeHtml(content.title || content.heading || content.word || content.phrase || content.missionTitle || "Listening") + '</h2>'
...
function buildListeningStandardCss() {
  return ""
