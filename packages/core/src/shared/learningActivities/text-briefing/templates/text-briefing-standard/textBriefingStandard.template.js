import { getTextBriefingDefaultContent } from "../../textBriefing.schema.js?v=1.1.226-learning-activity-files";

export function renderTemplate(activityContext) {
  var container = activityContext && activityContext.container ? activityContext.container : null;
  var content = activityContext && activityContext.content ? activityContext.content : getTextBriefingDefaultContent();

  if (!container) {
    return;
  }

  container.innerHTML = '<style>' + buildTextBriefingStandardCss() + '</style>'
    + '<article class="oqu-template-preview-shell">'
    + '<div class="oqu-template-preview-kicker">Text Briefing</div>'
    + '<h2>' + escapeHtml(content.title || content.heading || content.word || content.phrase || content.missionTitle || "Text Briefing") + '</h2>'
...
function buildTextBriefingStandardCss() {
  return ""
